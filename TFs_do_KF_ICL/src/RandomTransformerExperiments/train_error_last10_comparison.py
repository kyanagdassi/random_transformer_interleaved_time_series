"""
Compare GPT2 Pure vs GPT2 Modified on TRAINING data.
...
Aggregation: median over traces -> median over all configs
"""
import sys
import os
...
def evaluate_models_on_training_data(model_pure, model_modified, all_data, all_activations, 
                                     haystack_lengths, batches_so_far, k_values, device='cpu'):
    """
    Evaluate both models on training data accumulated so far.
    
    Training data: configs from batches 1 through batches_so_far (2 configs per haystack per batch)
    ...
    Aggregation: median over traces -> median over all configs (across all haystacks)
    
    Returns:
        aggregated_pure: dict[metric] = median error across all training configs
        aggregated_modified: dict[metric] = median error across all training configs
    """
    model_pure.eval()
    model_modified.eval()
    ...
    # Determine which configs to evaluate on (batches 1 through batches_so_far)
    # Batch i processes configs (i-1)*2 to i*2-1 per haystack (0-indexed)
    train_config_indices = list(range(batches_so_far * 2))  # e.g., batch 1: [0,1], batch 2: [0,1,2,3], etc.
    
    # Collect per-config medians from all training configs
...
def main():
    print("\n" + "="*80)
    print("TRAIN ERROR COMPARISON: GPT2 PURE VS MODIFIED (LAST 10 PREDICTIONS)")
    print("="*80 + "\n")
    ...
    text_path = os.path.join(output_dir, 'train_error_last10_results.txt')
    ...
    for batch_idx in range(num_batches):
        ...
        # Evaluate on training data accumulated so far (batches 1 through batch_idx+1)
        ...
        aggregated_pure, aggregated_modified = evaluate_models_on_training_data(
            ...
            all_activations,
            haystack_lengths,
            batch_idx + 1,  # Number of batches processed so far
            k_values,
            device
        )
    ...
    results_path = os.path.join(output_dir, "train_error_last10_results.pkl")
